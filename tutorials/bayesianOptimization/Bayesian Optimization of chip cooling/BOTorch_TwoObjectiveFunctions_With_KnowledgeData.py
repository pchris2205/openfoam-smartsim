...
sample_Non_Rotated = torch.tensor([20., 0., 0., 100., 100., 10., 10., 15., 15.])
train_x = torch.vstack([train_x, sample_Josip_Non_Rotated])
fname = 'chip_cooling_Not_Rotated/postProcessing/k_box/0/volFieldValue.dat'
data_k_box = np.loadtxt(fname, skiprows=4)
data_last_k_box = torch.tensor([data_k_box[-1,1]])
fname = 'chip_cooling_Not_Rotated/postProcessing/pressure_inlet/0/surfaceFieldValue.dat'
data_p_inlet = np.loadtxt(fname, skiprows=5)
data_last_p_inlet = data_p_inlet[-1,1]
data_pressure_drop = torch.tensor([-data_last_p_inlet])
data_final_Non_Rotated = torch.vstack([data_last_k_box, data_pressure_drop]).transpose(-1, -2)
train_y = torch.vstack([train_y, data_final_Non_Rotated])

sample_Rotated = torch.tensor([20., 45., 45., 100., 100., 10., 10., 15., 15.])
train_x = torch.vstack([train_x, sample_Josip_Rotated])
fname = 'chip_cooling_Rotated/postProcessing/k_box/0/volFieldValue.dat'
data_k_box = np.loadtxt(fname, skiprows=4)
data_last_k_box = torch.tensor([data_k_box[-1,1]])
fname = 'chip_cooling_Rotated/postProcessing/pressure_inlet/0/surfaceFieldValue.dat'
data_p_inlet = np.loadtxt(fname, skiprows=5)
data_last_p_inlet = data_p_inlet[-1,1]
data_pressure_drop = torch.tensor([-data_last_p_inlet])
data_final_Rotated = torch.vstack([data_last_k_box, data_pressure_drop]).transpose(-1, -2)
train_y = torch.vstack([train_y, data_final_Rotated])
...
torch.save(train_y, "Results_train_y_TwoObjectiveFunctions_With_KnowledgeData.pt")
torch.save(train_x, "Results_train_x_TwoObjectiveFunctions_With_KnowledgeData.pt")
torch.save(hypervolumes, "Results_hypervolumes_TwoObjectiveFunctions_With_KnowledgeData.pt")
