...
x_range_phi = np.linspace(0., 90., 19).astype(np.float64)
x_range_a = np.linspace(50., 200., 16).astype(np.float64)
x_range_n = np.linspace(1., 10., 10).astype(np.float64)
x_range_m = np.linspace(1., 10., 10).astype(np.float64)
bounds = [x_range_y_0, x_range_phi, x_range_a, x_range_n, x_range_m]


from smartsim import Experiment
exp = Experiment("GeometryOptimization_MultiObjective_reducedParameters")
# Read the number of mpi ranks from system/decomposePar dictionary
decompose_par = ParsedParameterFile("chip_cooling_with_reduced_parameters/system/decomposeParDict")
    ...
    for j in range(num_queries):
        ...
        with warnings.catch_warnings():
            ...
            next_x, acq_val = botorch.optim.optimize_acqf(
                policy,
                bounds = torch.tensor([[0.0, 0.0, 50.0, 1.0, 1.0], [70.0, 90.0, 200.0, 10.0, 15.0]]),
                q = 1,
                num_restarts = 20,
                raw_samples = 50
                )
...
torch.save(train_y, "Results_train_y_TwoObjectiveFunctions_reducedParameters.pt")
torch.save(train_x, "Results_train_x_TwoObjectiveFunctions_reducedParameters.pt")
torch.save(hypervolumes, "Results_hypervolumes_TwoObjectiveFunctions_reducedParameters.pt")
